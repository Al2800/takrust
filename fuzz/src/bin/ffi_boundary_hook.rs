...
fn main() {
    ...
    if !encode_input.is_empty() {
        ...
        if encode_status == RustakFfiStatus::Ok && !encoded.ptr.is_null() {
            ...
            let _ = unsafe { rustak_ffi_decode_tak_v1(encoded.ptr, encoded.len, &mut decoded) };
            let _ = unsafe { rustak_ffi_buffer_free(&mut decoded) };
        }

        let _ = unsafe { rustak_ffi_buffer_free(&mut encoded) };
    }

    let mut scratch = RustakFfiBuffer::default();
    let _ = unsafe { rustak_ffi_decode_tak_v1(data.as_ptr(), data.len(), std::ptr::null_mut()) };
    let _ = unsafe { rustak_ffi_encode_tak_v1(std::ptr::null(), data.len(), &mut scratch) };
    let _ = unsafe { rustak_ffi_buffer_free(&mut scratch) };
}
